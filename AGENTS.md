# plunk (GrowthPath fork)

GrowthPath's maintained fork of [useplunk/plunk](https://github.com/useplunk/plunk):
the multi-brand email platform (campaigns, contacts, templates, nurture
workflows) serving GrowthPath's products and agency customers. The adoption
program and operating decisions live in epic #9; read its `Current state`
before platform work. The adoption ADR is in business-works
`ventures/growthpath/decisions/`.

## Branch discipline

- `main` mirrors upstream. Keep GrowthPath-only commits out of it.
- `growthpath-mail` is the maintained production branch; all fork work lands
  here via PR.
- Feature branches: `fix/<issue>-<slug>` off `growthpath-mail`, numbered by
  this repo's issues.
- Every patch also gets a paired `upstream/<issue>-<slug>` branch cut clean
  against upstream `main`, PR'd to useplunk/plunk. Upstream acceptance lets us
  drop the fork-side patch at the next reconcile.
- Upstream reconcile: merge upstream into `growthpath-mail`. `CLAUDE.md` will
  conflict (symlink here, file upstream): keep the symlink, fold upstream's
  CLAUDE.md changes into the "Upstream guidance" section below, and update its
  source commit marker.

## Commands

```bash
yarn test:run && yarn lint        # fast check while iterating
yarn build && yarn test:run       # full verify before PR
```

Dev environment: `yarn services:up` (Postgres, Redis, Minio via Docker), then
`yarn dev`. Details in the upstream guidance below.

**Deploy proof:** the Railway production services must run the exact image
digest pinned in epic #9's `Current state`. A deployment reporting success is
not proof; compare the running digest.

## Issue classification

`type/bug` · `type/feature` · `type/task` · `type/debt`, `priority/p0`–`p2`,
and this repo's single area label `area/platform` (fork patches, deployment,
platform operations). One `--label` flag on creation.

## Operating rules

- A live workflow is immutable: duplicate, drain, switch.
- Agency customers never receive raw project keys; events go through the
  scoped ingest proxy.
- Every database upgrade starts with a verified backup.
- This repo is public (AGPL). No secrets, credentials, or customer data in
  code, issues, or fixtures.

## Upstream guidance

Verbatim from upstream `CLAUDE.md` (source: useplunk/plunk, fetched at fork
reconcile `ee0efe0`; title and intro lines dropped). Upstream-authored: treat
as their dev guidance, including notes about their environment.
